...
static void SetSysClockTo144_HSE(void){
  ...
  do{
    HSEStatus = RCC->CTLR & RCC_HSERDY;
    StartUpCounter++;
  } while((HSEStatus == 0) && (StartUpCounter < HSE_STARTUP_TIMEOUT));//599个时钟

  if((RCC->CTLR & RCC_HSERDY) != RESET){//HSE启动 

    RCC->CFGR0 = (uint32_t)(RCC_PLLSRC_HSE | //PLL选择HSE输入
    		RCC_PLLXTPRE_HSE | //HSE不分频输入到PLL
				RCC_PPRE1_DIV1 |//APB1为不分频
    		RCC_PPRE2_DIV2 |//APB2为2分频
				RCC_ADCPRE_DIV8|//PCLK2 8分频后作为 ADC 时钟
				RCC_PLLMULL18		//PLL 18 倍频输出
				);
    RCC->CTLR |= RCC_PLLON;//使能PLL
    while((RCC->CTLR & RCC_PLLRDY) == 0);//等待PLL配置 // 156个时钟
    RCC->CFGR0 |= RCC_SW_PLL;//系统时钟源选择PLL输入
//    while ((RCC->CFGR0 & (uint32_t)RCC_SWS) != (uint32_t)0x08);//等待系统时钟配置为PLL
  }
  /*HSE开启失败*/
  else{
  	StartUpCounter=0;
  	RCC->CTLR = RCC_HSION;
    do{
      HSEStatus = RCC->CTLR & RCC_HSIRDY;
      StartUpCounter++;
    } while((HSEStatus == 0) && (StartUpCounter < HSE_STARTUP_TIMEOUT));
    EXTEN->EXTEN_CTR |= EXTEN_PLL_HSI_PRE;//HSI不分频输入到PLL
    RCC->CFGR0 = (uint32_t)(RCC_PPRE1_DIV1 |//APB1为不分频
    		RCC_PPRE2_DIV2|//APB2为2分频
				RCC_ADCPRE_DIV8|//PCLK2 8分频后作为 ADC 时钟
				RCC_PLLMULL18//PLL 18 倍频输出
				);
    RCC->CTLR |= RCC_PLLON;//使能PLL
    while((RCC->CTLR & RCC_PLLRDY) == 0);//等待PLL配置 
    RCC->CFGR0 |= RCC_SW_PLL;//系统时钟源选择PLL输入
//    while ((RCC->CFGR0 & (uint32_t)RCC_SWS) != (uint32_t)0x08);//等待系统时钟配置为PLL
  }
}
