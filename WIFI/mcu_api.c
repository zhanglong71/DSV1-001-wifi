...
{
//    #error " 在串口接收中断中调用uart_receive_input(value),串口数据由MCU_SDK处理,用户 勿再另行处理, 后删除该行"
...
{
//    #error " 在需要一次缓存多个字节串口数据处调用此函数,串口数据由MCU_SDK处理,用户 勿再另行处理, 后删除该行"
...
{
 //   #error " 直接在main函数的while(1){}中添加wifi_uart_service(),调用该函数不要加任何条件判断, 后删除该行"
...
{
//    #error "  在main函数中添加wifi_protocol_init() wifi协议初始化,并删除该行"
    rx_buf_in = (unsigned char *)wifi_uart_rx_buf;
...
/**
 * @brief  MCU获取系统 ,用于校对本地时钟
 * @param  Null
 * @return Null
 * @note   MCU主动调用 后在mcu_write_rtctime函数内校对rtc时钟
 */
