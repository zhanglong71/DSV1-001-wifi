...
/******************************************************************************
                       1:修改产品信息
******************************************************************************/
#define PRODUCT_KEY "nr9beqegawqu1jm7"    //开发平台创建产品后生成的16位字符产品唯一标识
...
******************************************************************************/
//开关(只上报)
//备注:清扫开关为 虚拟开关，机器开机时候，会上报 ture
#define DPID_SWITCH_GO 1
//启动自清洁(可下发可上报)
//备注:自清洁开关仅仅在charging或者charging done,充电桩时才能使用，其他状态置灰
#define DPID_SELF_CLEAN 2
//模式(可下发可上报)
//备注:模式中，none表示没有选中任何一种模式，为消费者自定义的吸力，水量。none模式是必选项，禁止删除
#define DPID_MODE 3
//当前状态(只上报)
//备注:枚举默认值不可修改，不可删，否则会影响面板逻辑。可以在默认值后面自定义新增其他枚举值
//状态值尽量是状态的英文小写单词，如果需要增加故障中状态，则为in_trouble
#define DPID_STATUS 4
//吸力(可下发可上报)
//备注:默认值（可修改，删除，增加）
//gentle 温柔; normal 正常；strong强力；max超强
//状态值尽量是状态的英文小写单词，如果需要增加故障中状态，则为in_trouble
//机器在充电座上的时候，APP端不支持下发档位调节
#define DPID_SUCTION 8
//水量(可下发可上报)
//备注:默认值（可修改，删除，增加）
//close 关闭; low 低档；middle中档；high高档
//状态值尽量是状态的英文小写单词，如果需要增加故障中状态，则为in_trouble
//机器在充电座上的时候，APP端不支持下发档位调节
#define DPID_CISTERN 9
//剩余电量(只上报)
//备注:
#define DPID_BATTERY_PERCENTAGE 14
//故障告警(只上报)
//备注:可以增加或者删除报错故障，没有逻辑限制
